		...
		if (entity.onGround()) {
			for (int index0 = 0; index0 < 8; index0++) {
				...
				world.addParticle(ParticleTypes.SPIT,
						...
						(entity.level().clip(new ClipContext(entity.getEyePosition(1f), entity.getEyePosition(1f).add(entity.getViewVector(1f).scale(range)), ClipContext.Block.COLLIDER, ClipContext.Fluid.NONE, entity)).getBlockPos().getZ()), 0.2,
						0.2, 0.2);
				{
					...
					for (Entity entityiterator : _entfound) {
						if (!(entityiterator == entity || entityiterator instanceof ItemEntity || entityiterator instanceof ExperienceOrb
								...
								}.checkGamemode(entityiterator) || entityiterator instanceof DiamondTowerEntity)) {
