...
public class DianeBossEntityDiesProcedure {
	public static void execute(LevelAccessor world, Entity entity, Entity sourceentity) {
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator instanceof Player && !(entityiterator == entity)) {
						...
						if (!(sourceentity instanceof ServerPlayer _plr8 && _plr8.level() instanceof ServerLevel
								&& _plr8.getAdvancements().getOrStartProgress(_plr8.server.getAdvancements().getAdvancement(new ResourceLocation("craft_no_taizai:defeat_diane"))).isDone())) {
		...
		if (CraftNoTaizaiModVariables.MapVariables.get(world).Gideon) {
			...
			if (drop <= 5) {
				...
				CraftNoTaizaiModVariables.MapVariables.get(world).Gideon = true;
