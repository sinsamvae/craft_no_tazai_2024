...
public class EscanorBossOnEntityTickUpdateProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (entity.getPersistentData().getDouble("skill_cooldown") == 0 && !((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null)) {
			...
			if (distance <= 6) {
				if (ran == 1) {
					if (entity instanceof EscanorBossEntity) {
						((EscanorBossEntity) entity).setAnimation("attack_2");
					}
					CraftNoTaizaiMod.queueServerWork(8, () -> {
						{
							...
							if (!projectileLevel.isClientSide()) {
								...
								}.getArrow(projectileLevel, entity, 750, 1);
