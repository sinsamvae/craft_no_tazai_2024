...
public class IncreaseSpawnProcedure {
	@SubscribeEvent
			...
			if ((entity instanceof Projectile _projEnt ? _projEnt.getDeltaMovement().length() : 0) < 4) {
				for (int index0 = 0; index0 < 250; index0++) {
					if (world instanceof ServerLevel projectileLevel) {
						...
						}.getArrow(projectileLevel, entity, (float) (Math.ceil(0.45 * (entity.getCapability(CraftNoTaizaiModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftNoTaizaiModVariables.PlayerVariables())).ManaAttack) + 4), 1);
