...
public class WhirlShockProjectileHitsLivingEntityProcedure {
		...
		double delay = 0;
		{
			...
			for (Entity entityiterator : _entfound) {
				if (!(entity == entityiterator)) {
					entityiterator.setDeltaMovement(new Vec3(((-0.5) * entityiterator.getLookAngle().x), (1.5 * entityiterator.getLookAngle().y), ((-0.5) * entityiterator.getLookAngle().z)));
				}
			}
		}
